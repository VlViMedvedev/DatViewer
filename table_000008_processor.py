...
class Table000008Processor:
    def process_table(self, raw_data, table_widget):
        """
        Обрабатывает таблицу 000008 (Взвешивание) и отображает её в табличной части интерфейса.
        ...
        """
        print("Начата обработка таблицы 000008")
        table_code = "000008"
        table_lines = []
        inside_table = False
        ...
        for line in raw_data.splitlines():
            if line.startswith("DN") and table_code in line:
                ...
                print(f"Найдена строка TN, завершение блока таблицы 000008")
    ...
    def get_field_type(self, code):
        for field in TABLES_REFERENCE.get("DD000008", []):
            if field["code"] == code:
