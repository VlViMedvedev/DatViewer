...
class Table000006Processor:
    def process_table(self, raw_data, table_widget):
        """
        Обрабатывает таблицу 000006 (Перемещение животного) и отображает её в табличной части интерфейса.
        ...
        """
        print("Начата обработка таблицы 000006")
        table_code = "000006"
        table_lines = []
        inside_table = False
        ...
        for line in raw_data.splitlines():
            if line.startswith("DN") and table_code in line:
                ...
                print(f"Найдена строка TN, завершение блока таблицы 000006")
    ...
    def get_field_type(self, code):
        for field in TABLES_REFERENCE.get("DD000006", []):
            if field["code"] == code:
