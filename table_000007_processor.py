...
class Table000007Processor:
    def process_table(self, raw_data, table_widget):
        """
        Обрабатывает таблицу 000007 (Результат контрольной дойки) и отображает её в табличной части интерфейса.
        ...
        """
        print("Начата обработка таблицы 000007")
        table_code = "000007"
        table_lines = []
        inside_table = False
        ...
        for line in raw_data.splitlines():
            if line.startswith("DN") and table_code in line:
                ...
                print(f"Найдена строка TN, завершение блока таблицы 000007")
    ...
    def get_field_type(self, code):
        for field in TABLES_REFERENCE.get("DD000007", []):
            if field["code"] == code:
