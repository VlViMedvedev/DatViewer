...
from PyQt5.QtGui import QFont  # Добавляем импорт QFont


class Table000005Processor:
    def process_table(self, raw_data, table_widget):
        """
        Обрабатывает таблицу 000005 (Паспорт коровы или телки) и отображает её в табличной части интерфейса.
        ...
        """
        print("Начата обработка таблицы 000005")
        table_code = "000005"
        table_lines = []
        inside_table = False
        ...
        for line in raw_data.splitlines():
            if line.startswith("DN") and table_code in line:
                ...
                print(f"Найдена строка TN, завершение блока таблицы 000005")
    ...
    def parse_dn_line(self, dn_line):
        ...
        while start < len(dn_line):
            ...
            field = {
                "prefix": block[:2],  # Служебный префикс
                "code": block[2:8],  # Код столбца
                "length": int(block[8:10]),  # Длина значения
                "precision": int(block[10:11]),  # Точность
            }
            # Получаем тип поля (M или K) из справочника
            field["type"] = self.get_field_type(field["code"])
            fields.append(field)
    ...
    def get_field_type(self, code):
        """
        Получает тип поля (M, K или "") из справочника.

        :param code: Код поля
        :return: Тип поля (M, K или "")
        """
        for field in TABLES_REFERENCE.get("DD000005", []):
            if field["code"] == code:
    ...
    def get_field_name(self, code):
        """
        Получает текстовое название поля из справочника по коду.

        :param code: Код поля
        :return: Текстовое название поля
        """
        for field in TABLES_REFERENCE.get("DD000005", []):
            if field["code"] == code:
                return field["name"]
